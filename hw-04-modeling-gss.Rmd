...
```
***The mean area under the ROC curve is 0.62, while the overall accuracy across all cross-validation folds is 0.84.*** 
...
```
***The mean area under the ROC curve is 0.61, while the overall accuracy across all cross-validation folds is 0.83.*** 
...
***Model 1 performed slightly better*** 
...
ggplot(roc_combined, aes(x = 1 - specificity, y = sensitivity, color = model)) +
  ...
  labs(title = "ROC Curves for Both Models", x = "1 - Specificity", y = "Sensitivity")
```

***The AUC showed that model 2 worked better, which is the opposite that was shown in the training data.***
...
***Bootstrapping can be used to estimate the proportion of Americans who have been harassed by their bosses or co-workers by resampling from the existing survey data to create many simulated samples.*** 
...
***Based on the bootstrap analysis, we can say with 95% confidence that the true proportion of Americans who have experienced harassment by a superior or co-worker at work falls between 15.4% and 19.3%.*** 
...
***A 90% confidence interval would be narrower than the 95% confidence interval because lowering the confidence level means you're allowing for a greater chance that the interval will not contain the true population proportion. So, to compensate, the interval doesn’t need to stretch as far in either direction, it becomes more precise, but less confident.*** 
